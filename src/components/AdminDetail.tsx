...
export default function AdminDetail({ id }: { id: string }) {
  ...
  const summary = {
    ...
    scales: text({
      ...
      "премиальная — доступная": answerMap["character-premium-accessible"],
      "минималистичная — детализированная": answerMap["character-minimal-detailed"],
    }),
  ...
  return <main className="admin-detail">
    ...
    <section className="answer-groups">{editableSections.map((section) => <article key={section.id}><div className="answer-section-title"><span>{section.number}</span><h2>{section.title}</h2></div>{section.questions.map((question) => <div className="answer-row" key={question.id}><div><span className="mono">{question.number}</span><h3>{question.title}</h3></div><div>{question.type === "scale" && typeof answerMap[question.id] === "number" ? <div className="admin-scales"><div><strong>{answerMap[question.id] === "unknown" ? "Не могу определить" : String(answerMap[question.id])}</strong><i><b style={{ width: `${((Number(answerMap[question.id]) - 1) / 6) * 100}%` }} /></i></div></div> : question.type === "scales" && answerMap[question.id] && typeof answerMap[question.id] === "object" ? <div className="admin-scales">{Object.entries(answerMap[question.id] as Record<string, unknown>).map(([pair, value]) => <div key={pair}><span>{pair}</span><strong>{text(value)}</strong><i><b style={{ width: `${typeof value === "number" ? ((value - 1) / 6) * 100 : 0}%` }} /></i></div>)}</div> : <p>{text(answerMap[question.id])}</p>}</div></div>)}</article>)}</section>
